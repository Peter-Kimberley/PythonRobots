...
```python
# This imports the random module into the code.
import random
# This will track how many coin tosses they have been.
count = 0
# This will hold the value of the flip.
flip = 0

# The while loop creating the flip of a coin for seven turns.
while count <7:
    flip = random.random();
    if flip <= 0.5:
        print("The coin landed on tails")
    if flip > 0.5:
        print("The coin ladnded on heads")
    count = count + 1
    
    
```

    The coin ladnded on heads
...
*When the program is run the robot travels forward detecting the light value whilst displaying this to the output screen, it does this while it is detecting a value greater than 85, at the point that it sees a value that is less than 85 it exits the loop and prints the statement "I now see" and the value of the sensor reading that is less than 85.*
...
To answer this the robot would potentially continue in a forward motion.
...
# Test the sensor value; loop while the
# sensor value exceeds 77 this should cause the robot to stop at the third grey line.
while sensor_value > 77:
    # Display the sensor value in the simulator output window
    print(sensor_value)
    # Update the sensor_value variable
    # to the latest sensor reading
...
# The sensor value is now not greater than 50 
# so print a final message...
print("I now see {}".format( sensor_value))
...
*There is 2 recordings of the outputted text number for every 1 spoken count *
...
*Now we have introduced the wait=True parameter the say() command is now running in blocking mode rather than none blocking mode, this is giving a much more like for like match to what is said to the outputted text.*
...
*The robot travels past and off the gray banding, due to the blocking statement, the wait=True takes longer to complete, if this was not in place the code would execute past and this would give the program the ability to detect the third gray banded area in time and the resulting sensor value would cause the robots, motors to stop. *
