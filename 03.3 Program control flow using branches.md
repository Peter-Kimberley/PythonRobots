...
If you change the initial value then the code will not print the statement that "x equals 1" as this boolean is not true, the loop will not execute and the program will move to the print statement "All done...".
This means that the statements printed will be "Are you ready?" , "All done...".*
...
*I correctly predicted that this would happen based on the boolean for x = 1 not being True and the loop not being executed.*
...
*As the variable x = 1 is True the if statement is going to run the boolean and return True, so the print statement "x equals 1" is going to be displayed and the statement will be exited followed by the print statement "All done.." at the end.*
...
*This cell behaved as I expected as the first boolean returned as True*
...
__I think that the statement "x does not equal 1" will be printed as the x variable is == 2, therefore the first boolean check is going to return as False and the else statement will be run instead as per the design.
...
```
    ...
    Today is Sunday...
    ...the weekend
    And that's all the days of the week.
...
```
    ...
    ...Ruby Tuesday
    Today is Wednesday...
    ...I don't know a song title for that day
    Today is Thursday...
    ...I don't know a song title for that day
...
*The sensor value causes the robot to keep moving until the value of the sensor reads a <50 at that point the robot does not consider the reference to be anything but blacks*
...
*This program will cause the robot to move forward, if it detects a colour that is not white, it will reverse, turn and drive forward continually.*
...
*The robot will continue to say that it sees gray until the sensor sees black.*
